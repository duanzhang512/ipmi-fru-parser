...
INIHandler*  iniparser_loaddef(const char * def_ininame, const char * ininame)
{
    ...
    while ( (def_ini && (fgets(lin, ASCIILINESZ, def_ini) != NULL)) || (ini && (fgets(lin, ASCIILINESZ, ini) != NULL)) ) {
...
void iniparser_setstring(INIHandler *handler, char *hashstr, char * val)
{
    ...
    if(hashTable[hashval] == NULL) {
        ...
        if(!pnvp)
            return;
        pnvp->key = strdup(hashstr);
        pnvp->val = val ? strdup(val) : NULL;
        hashTable[hashval] = pnvp;
        return;
    }
    else{
